...
#endif
    [Export(typeof(ReplWindowProvider))]
    [PartCreationPolicy(CreationPolicy.Shared)]
    internal sealed class ReplWindowProvider : IReplWindowProvider {
        private readonly IInteractiveEvaluatorProvider[] _evaluators; //VSInteractiveWindowProvider [] _evaluators;
        private readonly Dictionary<int, NodejsInteractiveWindow> _windows = new Dictionary<int, NodejsInteractiveWindow>();

        [ImportingConstructor]
        public ReplWindowProvider([ImportMany]IInteractiveEvaluatorProvider[] evaluators) {
            _evaluators = evaluators;
        }

        #region IReplWindowProvider Members

        public IInteractiveWindow CreateReplWindow(IContentType contentType, string/*!*/ title, Guid languageServiceGuid, string replId) {
            int curId = 0;
            ...
            foreach (var provider in _evaluators) {
                var evaluator = provider.GetEvaluator(replId); //need a way to get a new Node
                if (evaluator != null) {
        ...
        public IInteractiveWindow FindReplWindow(string replId) {
        ...
        private IInteractiveWindow FindReplWindowInternal(int id) {
            NodejsInteractiveWindow res;
        ...
        public IInteractiveWindow CreateReplWindow(IInteractiveEvaluator/*!*/ evaluator, IContentType/*!*/ contentType, string[] roles, int id, string/*!*/ title, Guid languageServiceGuid, string replId) {
            return CreateReplWindowInternal(evaluator, contentType, roles, id, title, languageServiceGuid, replId);
        }

        private IInteractiveWindow CreateReplWindowInternal(IInteractiveEvaluator evaluator, IContentType contentType, string[] roles, int id, string title, Guid languageServiceGuid, string replId) {
            ...
            var replWindow = new NodejsInteractiveWindow(model, evaluator, contentType, roles, title, languageServiceGuid, replId);
            ...
            // seems to work fine.
            var creationFlags = __VSCREATETOOLWIN.activatewithproj;
